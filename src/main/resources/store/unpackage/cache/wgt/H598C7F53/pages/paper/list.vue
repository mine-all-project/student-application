<template>
	<div>
		<van-nav-bar left-text="文章分类" right-text="发表" left-arrow @click-left="back()" @click-right="addPaper">
		</van-nav-bar>
		...
		methods: {
			getFavorite(dataList) {
				axios.get(`${api.baseApi}/api/favorite/get`).then(response => {
					...
					const paperIds = result.data.paperIds.split(',')
					dataList.forEach(e => {
						paperIds.forEach(r => {
							if (e.id === r)
								e.favorite = 1
			...
			addFavorite(item) {
				axios.post(`${api.baseApi}/api/favorite/paper/add/${item.id}`).then(response => {
					const result = response.data
					if (result.status !== 200) {
						return;
					}
