<template>
	<el-row>
		<el-col :span="24">
			<el-row type="flex" class="row">
				<el-col :span="2">
					<div class="grid-content bg-purple"></div>
				</el-col>
				<el-col :span="4">
					<div class="grid-content bg-purple-light">文字文字:</div>
				</el-col>
				<el-col :span="10">
					<el-upload class="avatar-uploader" :action="`/api/uploadShopFile/tickets`" :submit="submit"
					           :show-file-list="false" ref="uploadFile" :on-success="successUpload">
						<img v-if="form.url" :src=`/file/${form.url}` class="avatar">
						<i v-else class="el-icon-plus avatar-uploader-icon"></i>
					</el-upload>
				</el-col>
			</el-row>

			<el-row type="flex" class="row">
				<el-col :span="2">
					<div class="grid-content bg-purple"></div>
				</el-col>
				<el-col :span="4">
					<div class="grid-content bg-purple-light">文字文字1:</div>
				</el-col>
				<el-col :span="10">
					<el-input size="small" v-model="form.sale"></el-input>
				</el-col>
			</el-row>

			<el-row type="flex" class="row">
				<el-col :span="2">
					<div class="grid-content bg-purple"></div>
				</el-col>
				<el-col :span="4">
					<div class="grid-content bg-purple-light"></div>
				</el-col>
				<el-col :span="10">
					<el-button type="danger" @click="submitShop">提交</el-button>
				</el-col>
			</el-row>
		</el-col>
	</el-row>

</template>

<script>
    module.exports = {
        data() {
            return {
                form: {
                    sale: '',
                    url: '',
                    keyWord: 'tickets',
                },
            };
        },
        mounted() {
            this.getShop()
        },
        methods: {
            getShop() {
                const _this = this;
                axios.post(`/api/saveAudioFile/${id ? id : 'create'}`, _this.drawer.form).then(response => {
                    const result = response.data;
                    console.log('通过api获取到的数据:', result);
                    if (result.status !== 200) {
                        this.$message.error('数据加载失败');
                        return
                    }
                    _this.content = result.data;
                    _this.$message.success('操作成功');
                    window.location.reload();
                }).catch(function (error) {
                    window.location.reload();
                    console.log('请求出现错误:', error);
                });
            },


            successUpload(res, file) {
                this.form.url = res.data.path
            },
            submitShop() {
            }
        }
    }
</script>

<style scoped>
	.row {
		margin-top: 20px;
	}

	.avatar-uploader .el-upload {
		border: 1px dashed #d9d9d9;
		border-radius: 6px;
		cursor: pointer;
		position: relative;
		overflow: hidden;
	}

	.avatar-uploader .el-upload:hover {
		border-color: #409EFF;
	}

	.avatar-uploader-icon {
		font-size: 28px;
		color: #8c939d;
		width: 178px;
		height: 178px;
		line-height: 178px;
		text-align: center;
	}

	.avatar {
		width: 178px;
		height: 178px;
		display: block;
	}
</style>
