...
//			MsgPushUtils.push(MsgPush.TYPE_SMS, title, content, null, null, receiveUserCode);
	...
	public String register(User user, HttpServletRequest request) {
		...
		if (!Global.TRUE.equals(userService.checkLoginCode(loginCode, loginCode/* , null */))) {
			return renderResult(Global.FALSE, text("保存用户失败，登录账号''{0}''已存在", loginCode));
		}
		// 非空数据校验。
		if (!(StringUtils.isNotBlank(loginCode) && StringUtils.isNotBlank(userName))) {
			return renderResult(Global.FALSE, "登录账号和用户姓名不能为空！");
		}
		// 新增并保存用户。
		User u = new User();
		u.setIsNewRecord(true);
		u.setUserCode(loginCode);
		u.setLoginCode(loginCode);
		u.setRefCode(loginCode);
		...
		if ("1".equals(userType)) {
			o.setOfficeCode("student");
			o.setOfficeName("学生");
		} else {
			o.setOfficeCode("company");
			o.setOfficeName("企业");
		}
		eu.setIsNewRecord(true);
		eu.setEmpCode(loginCode);
		eu.setEmpName(userName);
		eu.setOffice(o);
		employeeService.save(eu);
		// 添加角色
