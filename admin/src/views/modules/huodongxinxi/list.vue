  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
		...
		<el-form-item class="select" label="举办公司" prop="jubangongsi">
		  <el-select  @change="jubangongsiChange" clearable v-model="searchForm.jubangongsi" placeholder="请选择举办公司">
		    <el-option
			v-for="(item,index) in jubangongsiOptions"
			v-bind:key="index"
			:label="item"
			:value="item">
		    </el-option>
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('huodongxinxi','申请') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="huodongshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'申请':'' }}</el-button>
                <el-button v-if="isAuth('huodongxinxi','申请') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="huodongshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'申请':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('huodongxinxi','申请') && contents.tableBtnIcon == 0" type="success" size="mini" @click="huodongshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'申请':'' }}</el-button>
...
export default {
  ...
  methods: {
    ...
    init () {
          this.$http({
            url: `option/gongsixinxi/gongsimingcheng`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.jubangongsiOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        sort: 'id',
          ...
          if(this.searchForm.jubangongsi!='' && this.searchForm.jubangongsi!=undefined){
