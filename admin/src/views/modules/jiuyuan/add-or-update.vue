<template>
  <div class="addEdit-block">
    <el-form class="detail-form-content" ref="ruleForm" :model="ruleForm" :rules="rules"
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item class="select" label="4s店" prop="isdone">
            <el-select placeholder="请选择4s店" v-model="storeId" @change="changeStore">
              <el-option
                  v-for="(item,index) in dataList"
                  v-bind:key="index" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.ruleForm.content = this.ruleForm.content.replace(reg, '../../../springbooth5wle/upload');
        } else {
    ...
    onSubmit() {
        this.$http({
          url: `yuyue/finish/${this.tempid}/${this.storeId}`,
          method: "post",
        }).then(({data}) => {
          if (data && data.code === 0) {
