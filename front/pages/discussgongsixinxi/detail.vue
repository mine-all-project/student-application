...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussgongsixinxiCleanType')
			if(cleanType){
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussgongsixinxi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussgongsixinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussgongsixinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussgongsixinxi', this.detail);
				this.$utils.msg('审核成功');
