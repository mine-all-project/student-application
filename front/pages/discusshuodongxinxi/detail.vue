...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshuodongxinxiCleanType')
			if(cleanType){
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusshuodongxinxi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshuodongxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshuodongxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusshuodongxinxi', this.detail);
				this.$utils.msg('审核成功');
